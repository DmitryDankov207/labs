...
int BinaryTree::completeTask() {
    int count = 0;
    findSingleChildCount(m_root, &count);
    return count;
}

void BinaryTree::findSingleChildCount(BinaryTree::t_node *root, int *count)
{
    if((root->t_left && !root->t_right) ||  (root->t_right && !root->t_left))
        (*count)++;

    if(root->t_left != nullptr)
        findSingleChildCount (root->t_left, count);

    if(root->t_right != nullptr)
        findSingleChildCount (root->t_right, count);
}
