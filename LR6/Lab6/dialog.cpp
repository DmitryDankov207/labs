...
void Dialog::on_pushButton_swap_released()
{
    if(binTree.empty ())
        return;

    int minKey = binTree.minKey ();
    int maxKey = binTree.maxKey ();

    QString minVal = binTree.find (minKey);
    QString maxVal = binTree.find (maxKey);

    for(int i = 0; i < ui->tableWidget_input->rowCount (); i++) {

        if(ui->tableWidget_input->item (i, 0)->text () == QString::number (minKey)) {
            ui->tableWidget_input->item (i, 1)->setText (maxVal);
        }

        if(ui->tableWidget_input->item (i, 0)->text () == QString::number (maxKey)) {
            ui->tableWidget_input->item (i, 1)->setText (minVal);
        }
    }
